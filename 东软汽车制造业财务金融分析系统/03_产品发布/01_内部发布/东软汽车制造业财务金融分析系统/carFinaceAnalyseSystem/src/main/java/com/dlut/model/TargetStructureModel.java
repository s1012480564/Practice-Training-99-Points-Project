...
public class TargetStructureModel {
    ...
    public static HashMap retrieve(String code) throws Exception {
        ...
        List<TargetRelation> res=sqlSession.selectList("targetMapper.selRelaByCode",code);
        sqlSession.close();
    ...
    public static Boolean delete(String code,String subcode) throws Exception{
        if(!TargetModel.checkExists(code,subcode)) return false;
        InputStream resourceAsStream=Resources.getResourceAsStream("SqlMapConfig.xml");
        SqlSessionFactory sqlSessionFactory=new SqlSessionFactoryBuilder().build(resourceAsStream);
        SqlSession sqlSession=sqlSessionFactory.openSession();
